...
const char* ssid = "Bommesnet24GHz";  // Enter SSID here
const char* password = "00001111222233334444555566";  //Enter Password here
...
void loop() {
  ...
  while(Wire.available()){
    ir_left = Wire.read();
    ir_right = Wire.read();
    ultrasoon_right = Wire.read();
    ultrasoon_left = Wire.read();
    ultrasoon_under = Wire.read();
    ultrasoon_front = Wire.read();
    hall = Wire.read();
  }
  ...
  if (autoToggle && hall) {
    ...
    if (evade && !revert) {
      if (millis() - startEvade < 500) {
        dir = BACKWARD;
      } else if (millis() - startEvade < 4000) {
        dir = evadeDir;
      } else {
        evade = false;
        evadeCooldownMillis = millis();
    ...
    if (revert) {
      if (millis() - evadeCooldownMillis < 500) {
        dir = BACKWARD;
      } else if (millis() - evadeCooldownMillis < 6000) {
        dir = evadeDir;
      } else {
        revert = false;
      }
